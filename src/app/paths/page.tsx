      ...
      <div className="paths-root">
        <div className="paths-cover">
          ...
          <div className="paths-cta-row">
            <Link href="/playbook" className="paths-btn-1">Get step-by-step guide →</Link>
          ...
          {CATEGORIES.filter(c => filter === 'all' || filter === c.key).map(cat => (
            <div key={cat.key}>
                ...
                {cat.paths.map(p => {
                  ...
                  return (
                    <div key={p.num} className={`paths-card${isOpen ? ' open' : ''}`}>
                      <button type="button" className="paths-card-head" onClick={() => toggle(p.num)} aria-expanded={isOpen}>
              ...
              <div className="paths-section-cta">
                <Link href="/playbook" className="paths-btn-sc1">Get step-by-step guide →</Link>
