  ...
  return (
    <div className="home-page">
      <nav>
        ...
        <button className="nav-cta" onClick={startQuiz}>Find my archetype</button>
      </nav>
        ...
        <div id="landing" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', paddingTop: '62px' }}>
          ...
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', padding: '52px 24px 72px' }}>
            <div style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'center' }}>
              ...
              <button onClick={startQuiz} style={{ display: 'inline-flex', alignItems: 'center', gap: '9px', background: 'var(--ink)', color: '#fff', fontFamily: "'DM Sans', sans-serif", fontSize: '.95rem', fontWeight: 500, padding: '15px 34px', borderRadius: '50px', border: 'none', cursor: 'pointer' }}>
          ...
          <div className="container" style={{ maxWidth: '660px' }}>
            ...
            <div style={{ background: '#fff', borderRadius: '14px', border: '1px solid var(--border)', padding: '34px 30px', boxShadow: '0 2px 20px rgba(26,16,64,.07)', animation: 'fadeSlide .35s ease' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {q.opts.map((o, i) => (
                  <button key={i} onClick={() => pick(o.a as ArchetypeKey)}
                    style={{
...
function track(event: string, data?: Record<string, unknown>) {
  try {
    const payload = JSON.stringify({ event, data, page: '/', ts: Date.now() });
